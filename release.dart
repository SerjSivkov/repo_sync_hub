...
/// Тег создаётся на текущей ветке.
void main() async {
  print('📦 Repo Sync Hub Release Tool 📦');
  ...
  final suggestedPubspecBuild = currentBuild + 1;
  ...
  print('Enter new pubspec build number '
      '(or press Enter to use $suggestedPubspecBuild):');
  ...
  if (releaseType != 'stable') {
    print('Enter new tag build number '
        '(or press Enter to use $suggestedTagBuild):');
  ...
  try {
    await _runCommand('git', ['add', 'pubspec.yaml', 'assets/CHANGELOG.md']);
    print('✅ git add pubspec.yaml assets/CHANGELOG.md');
    ...
    print('Next: see docs/release-build.md for local macOS build & DMG.');
  } catch (e) {
...
Future<String> _defaultRemote() async {
  ...
  if (result.exitCode != 0) return 'origin';
